...
- **Beneficios**: el panel lista las promociones, packs y beneficios de las unidades elegidas, marcados por defecto y desmarcables. `POST /cotizaciones` **no los aplica solo**, así que sin esto la cotización salía a precio de lista — hoy 34 de 38 unidades disponibles tienen un precio webinar cargado. Se agregan con `POST /cotizaciones/:id/{promociones|packs|beneficios}`, que dispara `recalcularPromociones` en el backend; el frontend **no estima** el descuento, recarga la cotización y muestra los totales reales en el PDF
