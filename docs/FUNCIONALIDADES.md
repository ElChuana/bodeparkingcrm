...
- **Reingreso: fechas y campaña** (2026-08-25)
  - El reingreso **no crea un lead nuevo**: `creadoEn` mantiene la fecha de la primera vez y solo cambia `actualizadoEn`. Efecto que confunde: en la lista (ordenada por `actualizadoEn`) el lead salta arriba como reciente, pero los filtros por fecha van contra `creadoEn` y lo dejan fuera del rango nuevo. Ambos son correctos; juntos parecen un bug
  - **La campaña pasa a ser la última que lo trajo** (`lib/campanas.js: campanaTrasReingreso`, con tests). Antes solo se escribía si el lead no tenía ninguna, así que quien entró por el webinar de junio y se reinscribió en agosto seguía contando como lead de junio: no salía en el informe de la campaña nueva y su venta **no comisionaba como venta de webinar** (el motor mira `Campana.esWebinar`). El cambio queda escrito en el timeline (`Campaña: Webinar Junio 2026 → Webinar Agosto 2026`), así que el origen no se pierde
  - Aplica a `POST /leads` y al webhook del webinar. **Comuro queda fuera a propósito**: ahí la campaña es parte de la identidad del lead (`comuroController` busca el lead por `contactoId` + `campana`, o sea un lead por proyecto)
...
- Elegir un edificio **resetea el filtro de tipo a "Todas"**: quedarse con "Bodegas" del edificio anterior escondía los estacionamientos del nuevo (entrar a Brasil mostraba cero)
- **Beneficios** (el vendedor elige cuáles van, sin exhibirle el menú al cliente):
  - En el panel se ve **solo una línea**: "Incluye <los que van>". La lista completa está detrás de un lápiz discreto que abre un popover — tenerla desplegada le mostraba al cliente todo lo que podía pedir
